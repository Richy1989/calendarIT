...
public static class ICalEventMapper
{
    ...
    public static void Apply(ICalEvent ve, DomainEvent e, DateTime now)
    {
        ...
        // Only overwrite the stored color when the VEVENT carries one that resolves: most
        // CalDAV clients don't round-trip the RFC 7986 COLOR property, and an edit synced
        // from a phone must not wipe the color chosen in the web UI — including when the
        // value is a name (or garbage) we can't map to a hex.
        var colorHex = CssColorMap.ToHex(ve.Properties["COLOR"]?.Value?.ToString());
